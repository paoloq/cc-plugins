...
- **Harness-pillar grading**:
  - **Evals** (reads `static.evals`): `A` when `n_files ≥ plugins/skills count` and `n_total_cases ≥ 5`. `B` with at least one case file. `C` when `has_dir` but no parseable cases. `D` when `has_dir == false`.
